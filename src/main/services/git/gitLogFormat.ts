...
export const GIT_LOG_PRETTY_FORMAT = '%H %P%x1f%ai%x1f%an%x1f%ae%x1f%s%x1f%B%x1f%D%x1e';
...
export function parseGitLogOutput(output: string): GitLogEntry[] {
  ...
  return output
    ...
    .map((record) => {
      ...
      const [hash = '', ...parentParts] = (parts[0] || '').trim().split(/\s+/);
      ...
      return {
        hash,
        parents: parentParts.filter(Boolean),
        date: parts[1] || '',
        author_name: parts[2] || '',
        author_email: parts[3] || '',
        message,
        fullMessage,
        refs: refs ? refs.trim() || undefined : undefined,
      };
    });
}
